...
def analyze_swing_motion(cap: cv2.VideoCapture, fps: float, impact_frame: int):
    try:
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        motions = []
        brightness = []
        ...
        if impact_time_ms is not None:
            ...
            peak_time_ms = peak_idx / fps * 1000 if fps else 0
...
def main():
    ...
    requested_impact_frame = payload.get("impact_frame")
    impact_frame = (
        _to_int(requested_impact_frame, 0)
        if requested_impact_frame is not None
        else find_impact_frame(cap, detector, fps)
    )
    track_frames = max(2, _to_int(payload.get("track_frames"), 20))
    cap.set(cv2.CAP_PROP_POS_FRAMES, 0)  # reset for tracking
    trajectory = track_ball(cap, detector, impact_frame, track_frames=track_frames, fps=fps)
    swing = analyze_swing_motion(cap, fps, impact_frame)
    ...
    if len(trajectory) >= 2:
        ...
        shot_shape = "unknown"
        ball = {
            "vertical_launch_angle": None,
            "horizontal_launch_direction": None,
            "initial_velocity": None,
            "spin_bias": "unknown",
            "side_curve_intensity": None,
            "apex_height_relative": None,
            "side_deviation": None,
            "projected_carry_distance": None,
        }
        warning_code = "BALL_TRAJECTORY_INSUFFICIENT"
        warning_message = "공 궤적은 충분히 검출되지 않았지만 스윙 모션 분석은 완료했습니다."
        coach = [
            f"impact frame: {impact_frame}, tracked {len(trajectory)} pts",
            warning_message,
        ]
    ...
    if warning_code:
        result["warningCode"] = warning_code
