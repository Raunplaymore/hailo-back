...
| 구성요소 | 책임 | 기준 데이터 |
| --- | --- | --- |
| 모바일 웹앱 | 목록/상세/재생/삭제 UI | NAS library API |
| NAS library | 완료 job 보관, tailnet 접근 제어, 목록, 스트리밍, 삭제 이력 | `jobId`별 manifest와 artifact |
...
직접 공개하지 않는다. 모바일 기기의 Tailscale 인증이 library 접근 권한이므로 개인 사용자는 앱 로그인 없이
바로 이용한다. 공개 도메인이 필요해질 경우에만 별도 reverse proxy 또는 tunnel과 앱 로그인 세션을 추가한다.

### 권한 분리

| 주체 | 인증 수단 | 허용 작업 |
| --- | --- | --- |
| Pi archive client | `NAS_ARCHIVE_TOKEN` Bearer token | artifact/manifest 업로드, 삭제 이력 조회·확인 |
| 모바일 브라우저 | Tailscale tailnet 기기 인증 | 개인 library 조회, 영상 재생, 삭제 |
| NAS 내부 작업 | Docker 내부 환경변수 | 파일 시스템, index, tombstone 관리 |

`NAS_ARCHIVE_TOKEN`은 브라우저, Vite 환경변수, API 응답, manifest에 절대 포함하지 않는다. 현재
`storage-api`의 raw archive API는 Pi 전용 내부 API로 유지하며, 모바일은 tailnet으로 보호된 library API만 호출한다.
...
모든 `/api/library/*` endpoint는 tailnet 내부에서만 제공한다. NAS가 공개 인터넷에 노출되는 경우에만
`LIBRARY_AUTH_MODE=password`와 세션 로그인을 켠다.
...
| `GET` | `/v1/deletions?after=` | Pi 전용 tombstone 동기화 feed |
| `POST` | `/v1/deletions/:jobId/ack` | Pi의 로컬 삭제 반영 시각 기록 |
...
3. Pi가 다음 동기화에서 tombstone을 받고 로컬 영상, body, meta, analysis cache, shot record를 삭제한 뒤 `piSyncedAt` 확인을 NAS에 기록한다.
...
NAS storage API 컨테이너가 `/volume1/hailo/web/current`의 Vite production build를 함께 제공한다.
모바일의 기본 접속 주소는 NAS HTTPS 주소가 된다. `pi_web`의 NAS 전용 배포 workflow는 build를
release 디렉터리에 올리고 `current` 심볼릭 링크를 원자적으로 전환한다.
...
- NAS storage compose에 web build mount 추가
